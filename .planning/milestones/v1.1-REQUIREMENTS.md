# Requirements Archive: v1.1 AI Behavioral Awareness
...
- [x] **DATA-01**: Capture player text messages from group chat (message text, sender, timestamp) via middleware extension
- [x] **DATA-02**: Store last ~10 messages per player per game in dedicated DB table with automatic pruning
- [x] **DATA-03**: Filter out bot messages, non-player messages, DM messages, and commands before storage
- [x] **DATA-04**: Verify bot has admin status in group at game creation (required for message visibility)

### Behavioral Analysis

- [x] **BEHAV-01**: Compute per-player activity stats: message count, average length, time since last message, message frequency
- [x] **BEHAV-02**: Detect behavioral tone via heuristic keyword matching (accusatory, defensive, quiet, neutral, chaotic) using Swedish game-context keywords
- [x] **BEHAV-03**: Build compressed behavioral summaries (~50 tokens per player) that populate `GuzmanContext.playerNotes`
- [x] **BEHAV-04**: Detect behavioral anomalies: suspicious silence (active→quiet), aggression spikes, behavior shifts within a round

### AI Integration — Whispers

- [x] **WHISP-01**: Whisper prompts include target player's behavioral summary and 1-2 actual message quotes (paraphrased, never verbatim)
- [x] **WHISP-02**: Whisper prompts include all players' behavioral overview for context
- [x] **WHISP-03**: Prompt rules enforce oblique references only — Guzman speaks from "gut feeling," never quotes directly or references timestamps

### AI Integration — Group Messages

- [x] **GROUP-01**: Gap-fill commentary adapts to group mood (active/quiet, aggressive/cautious) using behavioral context
- [x] **GROUP-02**: Guzman publicly calls out suspicious behavior (silence, aggression) via accusation system piggybacked on gap-fill schedule
- [x] **GROUP-03**: Accusation frequency controlled: max 2 per round, never same player twice in a row *(adjusted from spec's "max 1 per 4-hour window" per user decision)*
...
**Adjusted:** GROUP-03 — spec said "max 1 per 4-hour window", implementation uses "max 2 per round" per locked user decision. Functionally equivalent frequency control.
**Dropped:** None
