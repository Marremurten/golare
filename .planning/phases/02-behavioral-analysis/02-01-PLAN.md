...
In the `updateNarrativeContext` function (starts at line 519), add the behavioral analysis try/catch block **immediately after the roundSummaries compression logic** (the `if (context.roundSummaries.length > 3)` block that ends with `context.roundSummaries = [...compressed, ...recent];` at line 563) **and before the final `await dbUpdateGuzmanContext(gameId, context)` call at line 566**. This ensures `playerNotes` and `behavioralHistory` are set on the `context` object before it is persisted.

The exact insertion point is between the closing brace of the compression block (line 564) and the `await dbUpdateGuzmanContext` call (line 566). Insert:
...
After this insertion, the function's operation sequence is:
1. Fetch context (line 526)
2. Append round summary (lines 529-534)
3. Update mood (lines 537-543)
4. Update story arc (lines 546-548)
5. Compress older summaries (lines 551-563)
6. **NEW: Behavioral analysis try/catch (populates playerNotes + behavioralHistory on context)**
7. Persist to DB (line 566: `await dbUpdateGuzmanContext(gameId, context)`)

This follows the existing error handling pattern in the codebase: try/catch with console.warn, non-blocking. The behavioralHistory is persisted alongside playerNotes in the same `dbUpdateGuzmanContext` call that already exists -- no additional DB write needed.
...
- Placing it after compression but before persist means the context object has all other updates (summary, mood, arc, compression) already applied, and the behavioral analysis results are included in the single DB write.
