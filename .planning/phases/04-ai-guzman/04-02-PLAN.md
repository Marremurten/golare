...
4. Call `const revealText = await generateResultReveal(round.round_number, guzmanContext, missionResult, golaCount, playerNames, teamNames)`
5. Send revealText instead of the separate SUSPENSE_1 + MISSION_SUCCESS/MISSION_FAIL sequence. The AI-generated reveal includes its own dramatic buildup, so the separate suspense messages are no longer needed when AI is active. If generateResultReveal falls back to template, keep the existing SUSPENSE_1 + result pattern. To detect fallback: generateResultReveal should return an object `{ text: string, isAI: boolean }` (adjust the ai-guzman.ts return type if needed, or check if guzmanContext has story arc).

Actually, simpler approach: Always send SUSPENSE_1 first (it's short and atmospheric), wait the sleep delay, then send the AI-generated result. This works for both AI and fallback paths consistently.

6. After the result is determined (success/fail/kaos_fail), call `await updateNarrativeContext(game.id, round.round_number, missionTheme, outcome, narrativeBeats)` where:
   - missionTheme: extract from AI response or use "Stöt {roundNumber}" as default
...
MISSION_POST_VARIANTS: [
  (roundNumber: number) => `<b>Runda ${roundNumber} -- ny stöt!</b> 🎯\n\n...`,
  (roundNumber: number) => `<b>Stöt nummer ${roundNumber}, bre.</b> 🔥\n\n...`,
  (roundNumber: number) => `<b>Ligan! Runda ${roundNumber}.</b> 🎯\n\n...`,  // Original
] as const,
```
Each variant should be in Guzman's voice, use proper åäö, and vary the opening/tone while conveying the same information: it's a new round, Capo picks team, group votes. Keep them shorter than AI narratives (~100-150 chars) -- these are "lazy Guzman" fallbacks.

3. Add MISSION_RESULT_VARIANTS similarly for both success and fail:
```typescript
MISSION_SUCCESS_VARIANTS: [
  "<b>Stöten lyckades!</b> ✅\n\nAlla var lojala -- eller så var Golare för fega att agera. Ligan tar poängen! 💰",
  "<b>Clean!</b> ✅\n\nIngen golade. Familjen håller ihop, bre. Poängen är vår. 💰",
  "<b>Lyckad stöt!</b> ✅\n\nBra jobbat. Inga råttor den här gången... eller? 🤔",
] as const,

MISSION_FAIL_VARIANTS: [
  (golaCount: number) => `<b>Stöten misslyckades!</b> ❌\n\n${golaCount} golade. Råttor bland oss, bre. 🐀`,
  (golaCount: number) => `<b>Saboterat!</b> ❌\n\n${golaCount} ${golaCount === 1 ? "person" : "personer"} sålde oss. Aina tar poängen. 🐀`,
  (golaCount: number) => `<b>Vi åkte dit!</b> ❌\n\nNågon golade -- ${golaCount} stycken. Familjen blöder, bre. 💀`,
] as const,
```
...
All Swedish text MUST use proper åäö characters. Never substitute with a/a/o.
  </action>
  <verify>
`npx tsc --noEmit` passes. `grep "FALLBACK_PREFIX\|MISSION_POST_VARIANTS\|getRandomVariant" src/lib/messages.ts` shows all new additions. Verify all Swedish text uses proper åäö (no "a" where "å" or "ä" should be).
  </verify>
  <done>3 variants each for mission post, mission success, and mission fail templates. Lazy Guzman fallback prefixes. getRandomVariant helper for variety in fallback mode. All text uses proper Swedish åäö characters.</done>
</task>
...
6. All Swedish text uses proper åäö characters
</verification>
