...
must_haves:
  truths:
    ...
    - "Accusations fire max 1 per round in rounds 1-2, max 2 per round in rounds 3+"
 ...
 * - Round 1-2: max 1 accusation per round
 * - Round 3+: max 2 accusations per round
...
function canAccuse(gameId: string, currentRound: number): boolean {
  ...
  const maxPerRound = currentRound <= 2 ? 1 : 2;
  return state.count < maxPerRound;
}
```
...
4. ACCUSATION ATTEMPT (takes priority over gap-fill):
   ...
   c. If target found: call generateAccusation(target.name, target.anomalies, target.evidence, playerNames, guzmanCtx, round.round_number)
...
3. `runGapFill` calls `analyzeBehavior`, `computeGroupMood`, `selectAccusationTarget`, `generateAccusation`
4. `generateGapFillComment` is called with 4 arguments (mood added)
5. Accusation delivery uses `queue.send` with `parse_mode: "HTML"`
6. No imports of `PlayerRole` in accusation path
  </verify>
  <done>
runGapFill delivers mood-adaptive gap-fill and piggybacked accusations. Frequency tracking enforces max 1 per round (rounds 1-2) or max 2 (rounds 3+). Same player never accused twice in a row. Accusation fires take priority over gap-fill (no double messages). Fresh behavioral data fetched at cron time with graceful fallback. All AI paths return null on failure (CONST-04).
  </done>
</task>
...
3. `canAccuse` enforces round-based limits: max 1 (rounds 1-2), max 2 (rounds 3+)
...
6. Accusation delivery skips gap-fill for that slot
7. `shouldSendGapFill` gates by mood: tense=always, active=when quiet, calm=never
8. `generateGapFillComment` receives mood string as 4th parameter
9. Fresh `analyzeBehavior()` at cron time with try/catch fallback to existing playerNotes
10. No player role information flows into accusation path
</verification>

<success_criteria>
Accusation system is fully wired: behavioral data flows from analyzeBehavior through mood computation and target selection into AI-generated accusations delivered via MessageQueue. Gap-fill commentary adapts to group mood. Frequency controls prevent spam. All AI paths degrade gracefully. The group chat will feel like Guzman is always watching.
</success_criteria>
