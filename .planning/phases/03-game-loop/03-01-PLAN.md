...
must_haves:
  truths:
    ...
    - "Scheduled events fire at 8 daily times on weekdays: 09:00, 11:00, 12:00, 14:00, 15:00, 17:00, 18:00, 21:00 Europe/Stockholm (5 core events from INFRA-06 + 3 reminder events per CONTEXT.md locked decision on 1h-before-deadline reminders)"
    ...
    - "sista_chansen table has UNIQUE constraint on game_id to enforce first-guess-wins atomically"
  artifacts:
...
2. **schema.sql** -- Append these new tables after the existing game_players table:
   ...
   - `sista_chansen` table: id (UUID PK), game_id (FK games ON DELETE CASCADE), guessing_side (TEXT -- 'golare'/'akta'), target_player_id (FK game_players), guessed_by_id (FK game_players), correct (BOOLEAN nullable), guessed_at timestamp. **CRITICAL: Add UNIQUE constraint on (game_id) to enforce first-guess-wins atomically at the database level.** Only one guess per game is allowed -- the INSERT will fail with a unique violation if a second guesser tries to submit.
...
4. **client.ts** -- Add these CRUD functions:
   ...
   - `createSistaChansen(game_id: string, guessing_side: GuessingSide, target_player_id: string, guessed_by_id: string): Promise<SistaChansen>` -- INSERT. **This will throw a unique violation error if a guess already exists for this game_id (UNIQUE constraint on game_id enforces first-guess-wins).** Callers must catch this error to handle the race condition gracefully.
  ...
  <done>
    All new types exported from types.ts, all new CRUD functions exported from client.ts, schema.sql has 4 new tables + join_order column (sista_chansen has UNIQUE on game_id), croner installed. TypeScript compiles cleanly.
  </done>
</task>

<task type="auto">
  <name>Task 2: Pure FSM and Croner scheduler</name>
  <files>
    src/lib/game-state.ts
    src/lib/scheduler.ts
  </files>
...
2. **src/lib/scheduler.ts** (NEW) -- Croner-based event scheduler:
   ...
   - `startScheduler(handlers: ScheduleHandlers): void` -- Creates 8 Cron jobs on Mon-Fri schedule:
     - 09:00 -> onMissionPost (INFRA-06 core event)
     - 11:00 -> onNominationReminder (1h before 12:00 deadline, per CONTEXT.md locked decision)
     - 12:00 -> onNominationDeadline (INFRA-06 core event)
     - 14:00 -> onVotingReminder (1h before 15:00 deadline, per CONTEXT.md locked decision)
     - 15:00 -> onVotingDeadline (INFRA-06 core event)
     - 17:00 -> onExecutionReminder (1h before 18:00 deadline, per CONTEXT.md locked decision)
     - 18:00 -> onExecutionDeadline (INFRA-06 core event)
     - 21:00 -> onResultReveal (INFRA-06 core event)

     NOTE: INFRA-06 specifies 5 core scheduled times (09:00, 12:00, 15:00, 18:00, 21:00). The 3 additional reminder times (11:00, 14:00, 17:00) implement the CONTEXT.md locked decision: "Reminders sent both via DM to the player who needs to act AND a group chat reminder, 1h before each deadline." These are not separate requirements but the implementation of reminder delivery at 1h before each deadline.
  ...
  <verify>
    Run `npx tsc --noEmit` and confirm zero type errors. Verify game-state.ts exports all FSM functions. Verify scheduler.ts exports start/stop/recover.
  </verify>
  <done>
    game-state.ts exports all FSM functions (nextRoundPhase, getCapoIndex, computeVoteResult, computeMissionResult, getTeamSize, getSistaChansenSide, checkWinCondition). scheduler.ts exports startScheduler (8 cron jobs: 5 core + 3 reminders), stopScheduler, recoverMissedEvents. TypeScript compiles cleanly.
  </done>
</task>

<task type="auto">
  <name>Task 3: Template messages, error variants, and bot wiring</name>
  <files>
    src/lib/messages.ts
    src/lib/errors.ts
    src/bot.ts
  ...
  <action>
1. **src/lib/messages.ts** -- Add all game loop templates after the existing status section. Use proper Swedish with aao characters. All templates in Guzman persona. Add these message keys:

   MISSION_POST: (roundNumber: number) => string -- "Ligan! Runda {N}. Det ar dags for en ny stot..." (template -- Phase 4 replaces with AI)
   ...
   SISTA_CHANSEN_DM: (targetDescription: string, playerNames: string[]) => string -- DM to guessers with explanation that first guess locks in the choice
   ...
   SUSPENSE_1: string -- "..." (for dramatic reveal)
   SUSPENSE_2: string -- Guzman tension building message
...
2. **src/lib/errors.ts** -- Add a GAME_LOOP_ERROR variant with 4-5 Swedish error messages.

3. **src/bot.ts** -- Wire up the scheduler:
  ...
  <done>
    messages.ts has ~30 new game loop templates (including SUSPENSE_1/2) with proper Swedish. errors.ts has GAME_LOOP_ERROR variant. Bot starts cleanly with scheduler and stub handlers wired in. TypeScript compiles with no errors.
  </done>
</task>
...
6. messages.ts has all ~30 new game loop templates (including SUSPENSE_1, SUSPENSE_2)
7. Bot starts and shuts down cleanly with scheduler
8. sista_chansen table in schema.sql has UNIQUE constraint on (game_id)
</verification>

<success_criteria>
The game loop foundation is complete: database schema supports rounds/votes/missions/sista-chansen (with atomic first-guess-wins constraint), TypeScript types cover all game loop entities, CRUD functions handle all database operations, FSM transitions are pure and testable, scheduler fires at 8 correct times (5 core + 3 reminders) with restart recovery, and all template messages are ready for use by Plans 02-04.
</success_criteria>
