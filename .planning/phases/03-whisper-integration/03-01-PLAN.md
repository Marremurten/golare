...
1. `selectQuotesForWhisper(messages: PlayerMessage[], count: number = 2): string[]`
   ...
   - Selection heuristic: prefer messages with most behavioral signal -- score by (a) message length (longer = more content), (b) keyword richness (flatten ALL values from the module-level TONE_KEYWORDS constant into a single string[] -- `Object.values(TONE_KEYWORDS).flat()` -- then count how many of those keywords appear in the message text), (c) presence of other player name mentions
...
Run `npx tsc --noEmit` to confirm no type errors. Verify both functions are exported by checking the file contains `export function selectQuotesForWhisper` and `export function buildAllPlayerOverview`.
  ...
  <done>
Two new pure functions exported from behavioral-analysis.ts: selectQuotesForWhisper selects highest-signal messages for whisper context (using flattened TONE_KEYWORDS values for keyword scoring), buildAllPlayerOverview builds a compressed multi-player summary string excluding the target.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
DISCRETION: Round-based escalation is implemented per Claude's discretion (CONTEXT.md allows planner to decide). This creates a natural narrative arc and solves the thin-data problem in early rounds.
...
IMPORTANT: All Swedish text must use proper characters. Double-check every string for correct usage of a, a, o / A, A, O.
  ...
  <verify>
Run `npx tsc --noEmit 2>&1 | grep -v 'ai-guzman\|whisper-handler'` to verify behavioral-analysis.ts and ai-prompts.ts compile correctly. Type errors in ai-guzman.ts and whisper-handler.ts are expected until Plan 02 updates their caller signatures. Manually inspect the prompt text to confirm: (1) no verbatim quote instructions exist, (2) gossip-dealer framing is present, (3) role calibration section exists with 3 role variants, (4) escalation section has 3 tiers, (5) all Swedish text uses proper characters.
  </verify>
  <done>
buildWhisperPrompt accepts 8 parameters (up from 4), produces a behavior-aware gossip-dealer prompt with role calibration, round escalation, quote paraphrasing rules, all-player overview, and strict WHISP-03 oblique-reference rules. Files modified in this plan (behavioral-analysis.ts and ai-prompts.ts) compile without errors. Caller mismatches in ai-guzman.ts and whisper-handler.ts are expected and resolved in Plan 02. Swedish text uses proper characters throughout.
  </done>
</task>
...
<verification>
- `npx tsc --noEmit 2>&1 | grep -v 'ai-guzman\|whisper-handler'` shows no errors (caller type mismatches in ai-guzman.ts and whisper-handler.ts are expected until Plan 02 updates them)
...
- Files modified in this plan compile without errors (caller mismatches in other files resolved in Plan 02)
